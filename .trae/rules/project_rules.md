...
- **Validation Strategy**: ALWAYS use dedicated Form Request classes (`php artisan make:request`) for validation. NEVER write validation logic directly inside the Controller. Include both validation rules and custom error messages.
- **Secure Updates**: In update methods, explicitly use `$request->safe()->except(['field_name'])` to filter out immutable fields (like dates or IDs). Do not rely on validation rules alone to protect these fields.
- **Data Access**: NEVER use `$request->all()`. STRICTLY use `$request->validated()` or `$request->safe()` to ensure only valid data is processed.
- **Response Handling**: STRICTLY use Laravel's native redirect methods with session flashing (`return redirect()->back()->with('success', 'Message');` or `->withErrors(...)`) for all form submissions. Do NOT return JSON responses or manually trigger toasts from the controller, as the frontend is already configured to detect and display session flash notifications automatically.
    - **Why this is better**:
        - **Cleaner Controllers**: Backend logic doesn't need to know how the notification is displayed (Toastr, SweetAlert, etc.), it just sets the session data.
        - **Automatic Error Bag**: Laravel automatically shares the `$errors` variable with every view, so frontend inputs highlight automatically without extra code.
...
### Layout Constraints
- **Guest & Auth Layouts**: Never use `max-w-*` constraints on sections when using Guest or Authenticated layouts. Sections should be allowed to be full width or controlled by the parent layout container.
