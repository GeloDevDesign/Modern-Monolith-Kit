...
const colors = [
    ...
    { name: 'Cyan', palette: { 50: '#ecfeff', 100: '#cffafe', 200: '#a5f3fc', 300: '#67e8f9', 400: '#22d3ee', 500: '#06b6d4', 600: '#0891b2', 700: '#0e7490', 800: '#155f75', 900: '#164e63', 950: '#083344' } },
...
const selectedColor = ref(colors.find(c => c.name === 'Lime')); // Default to Lime as per preset

onMounted(() => {
    // Initialize primary color from localStorage
    const storedColorName = localStorage.getItem('primaryColor');
    if (storedColorName) {
        const color = colors.find(c => c.name === storedColorName);
        if (color) {
            updatePrimaryColor(color);
        }
    } else {
        // Apply default (Lime) if nothing stored, just to be sure
        updatePrimaryColor(selectedColor.value);
    }
});
...
const updatePrimaryColor = (color) => {
    ...
    for (const [shade, value] of Object.entries(palette)) {
        document.documentElement.style.setProperty(`--p-primary-${shade}`, value);
    }
};
</script>
    ...
    <div class="flex flex-col gap-3">
        ...
        <div class="grid grid-cols-8 gap-2">
            <button 
                ...
                class="w-full aspect-square rounded-md flex items-center justify-center transition-all duration-200 outline-none focus:ring-2 focus:ring-offset-2 dark:focus:ring-offset-gray-900 border"
                :class="[
                    selectedColor.name === color.name 
                        ? 'border-gray-900 dark:border-white ring-2 ring-offset-1 ring-gray-900 dark:ring-white dark:ring-offset-gray-900' 
