        ...
        <div class="mx-auto flex flex-col gap-6">
            ...
            <div class="">
                ...
                <DataTable 
                    ...
                    :first="(props.backups.current_page - 1) * props.backups.per_page"
                ...
                >
                    ...
                    <Column field="date" header="Backup Date" sortable>
                        <template #body="slotProps">
                            <span class="font-medium">{{ slotProps.data.date }}</span>
                        </template>
                    </Column>

                    <Column field="type" header="Type" sortable>
                        <template #body="slotProps">
                            <Tag :value="slotProps.data.type" :severity="getSeverity(slotProps.data.type)" class="!text-[10px] !px-2 !py-0.5" />
                        </template>
                    </Column>

                    <Column field="size" header="File Size" sortable>
                        <template #body="slotProps">
                            <span class="font-mono text-xs">{{ slotProps.data.size }}</span>
                        </template>
                    </Column>
                    ...
                    <Column field="filename" header="File" sortable>
                        <template #body="slotProps">
                            <div class="flex items-center gap-2">
                                <i :class="slotProps.data.type === 'Database' ? 'pi pi-database text-blue-500' : 'pi pi-folder text-yellow-500'"></i>
                                <span class="text-gray-600 dark:text-gray-400 truncate max-w-[200px]" :title="slotProps.data.filename">
                    ...
                    <Column header="Actions" class="w-32">
                        <template #body="slotProps">
                            <div class="flex gap-2">
                                <Button icon="pi pi-download" severity="secondary" text rounded aria-label="Download" @click="downloadBackup(slotProps.data)" v-tooltip.top="'Download'" />
                                <Button icon="pi pi-trash" severity="danger" text rounded aria-label="Delete" @click="deleteBackup(slotProps.data)" v-tooltip.top="'Delete'" />
                            </div>
                        </template>
                    </Column>
                </DataTable>
