...
- **Attribute Naming**: When setting a default value for a class attribute (typically used as a constant or configuration), use `UPPER_SNAKE_CASE` with a type prefix if applicable. For example: `protected int $INT_PER_PAGE = 15;` instead of `protected int $perPage = 15;`.
